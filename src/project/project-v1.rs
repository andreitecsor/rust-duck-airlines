    ...
    for waypoint in route.iter() {
        match previous_waypoint {
            ...
            Some(previous_waypoint_value) => {
                ...
                previous_waypoint = Option::from(waypoint.clone());
                println!("The distance between {} and {} is {:.2} km", previous_waypoint_value.0, waypoint.0, distance);
            }
        }
    }
    println!("Total distance between the two point is {} km", total_distance);
}
